...
# current/2040
ptm = proc.time()
res_current_2040 = foreach(j = 1:length(files),.packages=c('raster', 'viridis'), .combine = rbind) %dopar% {
...
# current/2070
ptm = proc.time()
res_current_2070 = foreach(j = 1:length(files),.packages=c('raster', 'viridis'), .combine = rbind) %dopar% {
...
# 2040/2070
ptm = proc.time()
res_2040_2070 = foreach(j = 1:length(files),.packages=c('raster', 'viridis'), .combine = rbind) %dopar% {
...
```{r}
res = single_species_comparison(models, files[2], country, 2040, 2070, F)
```
